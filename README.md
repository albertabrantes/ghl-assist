...
> **v1:** This release ships the skill itself plus the two-layer knowledge-base workflow. Point it at a local clone of the [official GHL OpenAPI specs](https://github.com/GoHighLevel/highlevel-api-docs) for instant endpoint reference (see [The knowledge base](#the-knowledge-base-optional-but-recommended) below). A bundled set of enriched, human-readable endpoint docs is on the way in a follow-up release.

Use it when you're:

- Writing code that calls GHL endpoints
- Building n8n (or other) workflows that integrate with GHL
