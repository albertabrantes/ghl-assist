...
- **`MONETORY` spelling:** The custom field `dataType` value for currency fields is spelled `MONETORY`, not `MONETARY`. This is a typo baked into the API — use it exactly as spelled, do not "correct" it. (Verified present in live custom-field responses.)
